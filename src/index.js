...
function deleteTextNodesRecursive(where) {
    for (var i = where.childNodes.length - 1; i >= 0; i--) {
        ...
        } else if (where.childNodes[i].nodeType == Node.TEXT_NODE) {
            where.removeChild(where.childNodes[i]);
        }
    }
}
    ...
    function statsCollector(el) {
        for (var i = 0; i < el.childNodes.length; i++) {
            if (el.childNodes[i].nodeType == Node.TEXT_NODE) {
                ...
                stats.tags[tag] = tag in stats.tags ? ++stats.tags[tag] : 1;
                el.childNodes[i].classList.forEach(function(cName) {
                    stats.classes[cName] = cName in stats.classes ? ++stats.classes[cName] : 1;
                })
                statsCollector(el.childNodes[i]);
            }
        }
    }
    statsCollector(root);
